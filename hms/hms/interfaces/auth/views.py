...
class AuthenticateUserView(viewsets.ViewSet):
    ...
    # NOTE: Add condition like get_serializer_class
...
class PasswordHandlerView(viewsets.ViewSet):
    ...
    def forgot_password(self, request, *args, **kwargs):
        ...
        try:
            serializer = serializer(data=request.data)
            if serializer.is_valid():
                user = self.user_app_service.get_user_by_email(
    ...
    def verify_otp(self, request, pk, *args, **kwargs):
        ...
        try:
            ...
            if user:
                ...
                if serializer.is_valid():
                    # run verify otp here
                    otp = self.user_app_service.verify_otp(
                        serializer.data["otp"], user.id
                    )
                    if otp:
                        response_data = {
                            "reset": f"{reverse('pwd-reset-password', kwargs={'pk':user.id})}?token={otp.otp_token}"
                        }
                        return CustomResponse(data=response_data).success_message()
            ...
            return CustomResponse(message="Your OTP has expired!").error_message()
        except Exception as e:
            return CustomResponse(message=e).error_message()
    ...
    def reset_password(self, request, pk, *args, **kwargs):
        ...
        try:
            user = self.user_app_service.get_active_user_by_id(id=pk)
            if user:
                new_password = {"new_password": request.data.get("new_password")}
                data = {
                    "user_id": user.id,
    ...
    def change_password(self, request, *args, **kwargs):
        ...
        try:
            ...
            if (
                user
                and old_password
                and authenticate(email=user.email, password=old_password)
            ):
                serializer = NewPasswordSerializer(data=request.data)
                if serializer.is_valid():
                    self.user_app_service.set_new_password(
                        user_id=user.id,
                ...
                return CustomResponse(data=serializer.errors).error_message()
            return CustomResponse(
                message="No User Found! Please provide correct old_password."
            ).error_message()
        except Exception as e:
            return CustomResponse(message=e).error_message()
